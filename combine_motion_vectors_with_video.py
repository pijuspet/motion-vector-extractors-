...
# Usage: python combine_motion_vectors_with_video.py <input_video> <csv_file> [video_segment_index] [max_frames]
if len(sys.argv) < 3:
    print("Usage: python combine_motion_vectors_with_video.py <input_video> <csv_file> [video_segment_index] [max_frames]")
    sys.exit(1)
...
# Video parameters (can be adjusted or auto-detected)
frame_width = 320
frame_height = 240
fps = 30
...
if len(motion_dfs) > 0:
    ...
    num_segments = 1  # Only video
combined_width = frame_width * num_segments

# By default, place the video in the last segment, or use user-supplied index
if len(sys.argv) > 3:
    video_segment_index = int(sys.argv[3])
else:
    video_segment_index = len(motion_dfs)  # last segment
...
# Determine max number of frames (synchronize between CSVs and video)
...
# Output video width is width per segment * (number of methods + 1 for video)
num_segments = len(motion_dfs) + 1
combined_width = frame_width * num_segments
...
for frame_num in range(1, num_frames + 1):
    ...
    # Read frame from input video if this segment is designated for video
    ...
    for i in range(num_segments):
        ...
        # Optional: Draw vertical dividing line between segments
        if i > 0:
