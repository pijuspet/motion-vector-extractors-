...
if len(sys.argv) < 2:
    print("Usage: python3 mv_python_extractor5.py <input.mp4 or rtsp>", file=sys.stderr)
    sys.exit(1)
...
frame = 0
print("frame,method_id,source,w,h,src_x,src_y,dst_x,dst_y,flags,motion_x,motion_y,motion_scale")
       ...
       #print(f"[DEBUG] Frame {frame} - {len(mvs)} motion vectors", file=sys.stderr)
        ...
        for v in mvs:
            try:
                ...
                if len(v) >= 9:
                    print(f"{frame},5,{v[0]},{v[1]},{v[2]},{v[3]},{v[4]},{v[5]},{v[6]},0x0,{v[7]},{v[8]},1")
                    v = v
                else:
