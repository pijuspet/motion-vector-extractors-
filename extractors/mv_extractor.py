...
if len(sys.argv) < 2:
    print(
        "Usage: python3 mv_python_extractor5.py <input.mp4 or rtsp> <output.csv>",
        file=sys.stderr,
    )
    sys.exit(1)
...
f.write(
    "frame,method_id,source,w,h,src_x,src_y,dst_x,dst_y,flags,motion_x,motion_y,motion_scale\n"
)
...
while True:
    try:
        ...
        if not result or not isinstance(result, tuple):
            print(
                f"[ERROR] cap.read() returned non-tuple or empty at frame {frame}: {result}",
                file=sys.stderr,
            )
            break
        ...
        if len(result) < 3:
            print(
                f"[ERROR] cap.read() tuple too short at frame {frame}: {result}",
                file=sys.stderr,
            )
            break
        ...
        for v in mvs:
            try:
                ...
                if len(v) >= 9:
                    f.write(
                        f"{frame},5,{v[0]},{v[1]},{v[2]},{v[3]},{v[4]},{v[5]},{v[6]},0x0,{v[7]},{v[8]},1\n"
                    )
                else:
                    print(
                        f"[WARN] Skipping malformed MV at frame {frame}: {v}",
                        file=sys.stderr,
                    )
            except Exception as mv_err:
                print(
                    f"[ERROR] Exception printing MV at frame {frame}: {mv_err}",
                    file=sys.stderr,
                )
